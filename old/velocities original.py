...
TODO: split into vanilla density field and field with physical velocities.
...
class Field(object):
    ...
    def v_k(self):
        ...
        if self._v_k is None:
            ...
            for i in range(ndim):   
                dipole_i = np.random.standard_normal()*self.dipole_rms/math.sqrt(ndim)
    ...
    def catalog(self, nbar, verbose=False, lowmem=False):
        ...
        ### extract the 3d velocities -- this does rely on 3d, but can be changed...
        ndim = idxs.shape[1]

        ### this can definitely be done in a single line...
        if ndim == 3: 
            vels = np.array([self.v_r[:, idxs[i][0], idxs[i][1], idxs[i][2]] for i in range(len(idxs))])
        elif ndim == 2:
...
def processCatalog(posns, vels, verr=0, nonlin=0, ctr=(0,0,0), lens=(1,1,1)):
    """
       ...
       convert 3d velocity catalog to radial velocities. 
    ...
    ## center positions
    lens = np.array(lens)
    pos = np.mod(posns-np.array(ctr), lens)-lens/2.0
    
    nl_vels = vels.copy()
    if nonlin > 0:
        nl_vels += np.random.standard_normal(vels.shape)*nonlin/math.sqrt(3)
...
def generate_catalog(N=256, nbar=500, length=100, verr=1000, nonlin=250,
    ...
    field = Field(delta_k=fk, deltas=deltas, dipole_rms=dipole_rms)
    
    pos, vel = field.catalog(nbar, verbose=True, lowmem=True)
    
    if not len(pos):
        raise Exception("Uh oh, no points")
    
    pos, vr = processCatalog(pos, vel, lens=length, verr=verr, nonlin=nonlin)
...
def main(options=None):
    ...
    generate_catalog(N=options.npoints, nbar=options.nbar, length=options.length, 
